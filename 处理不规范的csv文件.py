...
class CSVImporter:
    ...
    def setup_ui(self):
        ...
        # 设置滚动条样式
        style = ttk.Style()
        style.configure("Thick.TScrollbar", thickness=20, arrowsize=20, background="#4CAF50", 
                        troughcolor="#e1e1e1", borderwidth=2, relief="raised")
        v_scrollbar.configure(style="Thick.TScrollbar")
        h_scrollbar.configure(style="Thick.TScrollbar")
        ...
        self.preview_text = scrolledtext.ScrolledText(preview_frame, height=10, wrap=tk.NONE)
    ...
    def detect_csv_format(self, file_path):
        try:
            ...
            messagebox.showinfo("格式检测", f"已自动检测CSV格式:\n分隔符: {repr(delimiter)}\n文本限定符: {repr(quotechar)}")
